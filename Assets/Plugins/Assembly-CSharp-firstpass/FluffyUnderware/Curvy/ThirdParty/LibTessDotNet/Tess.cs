...
namespace FluffyUnderware.Curvy.ThirdParty.LibTessDotNet
{
	public class Tess
	{
		...
		private void OutputPolymesh(ElementType elementType, int polySize)
		{
			...
			ArrayPool<int> pool = ArrayPoolsProvider.GetPool<int>();
			if (elementsList.HasValue)
			{
				pool.Free(elementsList.Value);
			}
			elementsList = pool.Allocate(num * polySize);
			_vertexCount = num2;
			...
			for (MeshUtils.Face next2 = _mesh._fHead._next; next2 != _mesh._fHead; next2 = next2._next)
			{
				if (next2._inside && (!NoEmptyPolygons || !(Math.Abs(MeshUtils.FaceArea(next2)) < float.Epsilon)))
				{
					...
					{
						MeshUtils.Vertex next = edge._Org;
						elementsList.Value.Array[num4++] = next._n;
						num3++;
						edge = edge._Lnext;
					}
					while (edge != next2._anEdge);
					...
					{
						elementsList.Value.Array[num4++] = -1;
					}
					if (elementType == ElementType.ConnectedPolygons)
					{
						edge = next2._anEdge;
						do
						{
							elementsList.Value.Array[num4++] = GetNeighbourFace(edge);
							edge = edge._Lnext;
						}
						while (edge != next2._anEdge);
						for (int i = num3; i < polySize; i++)
		...
		private void OutputContours()
		{
			...
			elementsList = pool.Allocate(_elementCount * 2);
			_vertices = new ContourVertex[_vertexCount];
			int num3 = 0;
			int num4 = 0;
			num = 0;
			for (MeshUtils.Face next = _mesh._fHead._next; next != _mesh._fHead; next = next._next)
			{
				if (next._inside)
				{
					...
					while (edge != edge2);
					elementsList.Value.Array[num4++] = num;
					elementsList.Value.Array[num4++] = num2;
					num += num2;
				}
			}
		}
