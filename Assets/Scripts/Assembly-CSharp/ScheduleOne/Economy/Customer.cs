...
namespace ScheduleOne.Economy
{
	...
	public class Customer : NetworkBehaviour, ISaveable
	{
		...
		public const int DEAL_ATTENDANCE_TOLERANCE = 15;
		...
		//	protected set
		//	{
		//		syncVar____003CCurrentAddiction_003Ek__BackingField.SetValue(value, true);
		//	}
		//}
		...
		public float CurrentAddiction
		{
			get
			{
				return _003CCurrentAddiction_003Ek__BackingField;
			}
			protected set
			{
				_003CCurrentAddiction_003Ek__BackingField = value;
		...
		public float SyncAccessor__003CCurrentAddiction_003Ek__BackingField
		{
			get
			{
				return syncVar____003CCurrentAddiction_003Ek__BackingField.GetValue(calledByUser: true);
			}
			set
			{
				...
				{
					syncVar____003CCurrentAddiction_003Ek__BackingField.SetValue(value, value != 0);
				}
			}
		}
		...
		protected override void OnValidate()
		{
			...
			if (DealSignal == null)
			{
				NPCAction nPCAction = GetComponent<NPC>().behaviour.ScheduleManager.ActionList.Find((NPCAction x) => x.GetType() == typeof(NPCSignal_WaitForDelivery));
				if (nPCAction == null)
				{
					GameObject obj = new GameObject("DealSignal");
					obj.transform.SetParent(GetComponent<NPC>().behaviour.ScheduleManager.transform);
		...
		private void SetUpDialogue()
		{
			...
			sampleChoice.onChoosen.AddListener(DirectApproachOffered);
			sampleChoice.shouldShowCheck = ShowDirectApproachOption;
			sampleChoice.isValidCheck = DirectApproachOptionValid;
			NPC.dialogueHandler.GetComponent<DialogueController>().AddDialogueChoice(sampleChoice);
			...
			NPC.dialogueHandler.GetComponent<DialogueController>().AddDialogueChoice(completeContractChoice);
		...
		private void UpdateDealAttendance()
		{
			...
			int num2 = ScheduleOne.GameTime.TimeManager.AddMinutesTo24HourTime(CurrentContract.DeliveryWindow.WindowStartTime, 15);
			...
			{
				int value = Mathf.CeilToInt(num / NPC.Movement.WalkSpeed * 1.5f);
				value = Mathf.Clamp(value, 15, 360);
				int min = ScheduleOne.GameTime.TimeManager.AddMinutesTo24HourTime(windowStartTime, -value);
		...
		private ContractInfo CheckContractGeneration(bool force = false)
		{
			...
			correspondingQuality = EQuality.Poor;
			...
			return new ContractInfo(Mathf.RoundToInt(num4 * (float)value / 5f) * 5, new ProductList
			{
				entries = 
				{
					new ProductList.Entry
					{
			...
			}, deliveryWindow: new QuestWindowConfig
			{
				IsEnabled = true,
				WindowStartTime = 0,
				WindowEndTime = 0
			}, deliveryLocationGUID: DefaultDeliveryLocation.GUID.ToString(), expires: true, expiresAfter: 1, pickupScheduleIndex: 0, isCounterOffer: false);
		}
		...
		{
			potentialCustomerPoI.enabled = !NPC.RelationData.Unlocked && NPC.RelationData.IsMutuallyKnown();
		}
		...
		public virtual void ProcessHandover(HandoverScreen.EHandoverOutcome outcome, Contract contract, List<ItemInstance> items, bool handoverByPlayer, bool giveBonuses = true)
		{
			...
			NetworkObject dealer = null;
			if (contract.Dealer != null)
			{
				dealer = contract.Dealer.NetworkObject;
			}
			ProcessHandoverServerSide(outcome, items, handoverByPlayer, contract.Payment + num, contract.ProductList, satisfaction, dealer);
		}
		...
		[ObserversRpc]
		private void ProcessHandoverClient(float satisfaction, bool handoverByPlayer)
		{
			RpcWriter___Observers_ProcessHandoverClient_844349721(satisfaction, handoverByPlayer);
		}

		public void ContractWellReceived(bool handoverByPlayer)
		{
			if (GameManager.IS_TUTORIAL && NetworkSingleton<VariableDatabase>.Instance.GetValue<float>("Completed_Contracts_Count") >= 2.9f)
			{
				NPC nPC = NPCManager.GetNPC("chelsey_milson");
				RecommendFriend(nPC.GetComponent<Customer>());
			}
			else if (NPC.RelationData.NormalizedRelationDelta > 0.7f && UnityEngine.Random.value > 0.75f && CanRecommendFriends && NPC.RelationData.GetLockedConnections(onlyCustomers: true).Count > 0 && handoverByPlayer && !GameManager.IS_TUTORIAL)
			{
				List<NPC> lockedConnections = NPC.RelationData.GetLockedConnections(onlyCustomers: true);
				NPC nPC2 = lockedConnections[UnityEngine.Random.Range(0, lockedConnections.Count - 1)];
				RecommendFriend(nPC2.GetComponent<Customer>());
			}
			else
			{
		...
		private void RecommendFriend(Customer friend)
		{
			Console.Log("Customer " + NPC.fullName + " recommended friend " + friend.NPC.fullName + " to player");
			friend.SetHasBeenRecommended();
			string dialogueText = dialogueDatabase.GetLine(EDialogueModule.Customer, "post_deal_recommend").Replace("<NAME>", friend.NPC.fullName);
			DialogueContainer container = new DialogueContainer();
		...
		public ScheduleOne.Persistence.Datas.CustomerData GetCustomerData()
		{
			...
			return new ScheduleOne.Persistence.Datas.CustomerData(SyncAccessor__003CCurrentAddiction_003Ek__BackingField, array, array2, TimeSinceLastDealCompleted, TimeSinceLastDealOffered, OfferedDeals, CompletedDeliveries, OfferedContractInfo != null, OfferedContractInfo, OfferedContractTime, TimeSincePlayerApproached, TimeSinceInstantDealOffered, HasBeenRecommended);
		}
		...
		protected virtual bool DirectApproachOptionValid(out string invalidReason)
		{
			...
			if (GetDirectApproachSuccessChance() == 0f)
			{
		...
		public void DirectApproachOffered()
		{
			if (awaitingSample)
			{
				DirectApproachAccepted();
				return;
			}
			float directApproachSuccessChance = GetDirectApproachSuccessChance();
			if (UnityEngine.Random.Range(0f, 1f) <= directApproachSuccessChance)
			{
				DirectApproachAccepted();
				return;
			}
		...
		protected virtual float GetDirectApproachSuccessChance()
		{
			if (NPC.RelationData.Unlocked)
			{
				return 1f;
		...
		protected virtual void DirectApproachAccepted()
		{
			awaitingSample = true;
			NPC.dialogueHandler.SkipNextDialogueBehaviourEnd();
			NPC.PlayVO(EVOLineType.Acknowledge);
		...
		public float GetProductEnjoyment(ProductDefinition product, EQuality quality)
		{
			...
			{
				num += currentAffinityData.GetAffinity(product.DrugTypes[j].DrugType) * 0.3f;
			}
			float num2 = 0f;
			int i;
			for (i = 0; i < customerData.PreferredProperties.Count; i++)
			{
				if (product.Properties.Find((ScheduleOne.Properties.Property x) => x == customerData.PreferredProperties[i]) != null)
				{
		...
		public virtual void NetworkInitialize___Early()
		{
			if (!NetworkInitialize___EarlyScheduleOne_002EEconomy_002ECustomerAssembly_002DCSharp_002Edll_Excuted)
			{
				...
				RegisterObserversRpc(12u, RpcReader___Observers_ProcessHandoverClient_844349721);
		...
		private void RpcLogic___ProcessHandoverServerSide_3760244802(HandoverScreen.EHandoverOutcome outcome, List<ItemInstance> items, bool handoverByPlayer, float totalPayment, ProductList productList, float satisfaction, NetworkObject dealer)
		{
			...
			ProcessHandoverClient(satisfaction, handoverByPlayer);
		}
		...
		private void RpcWriter___Observers_ProcessHandoverClient_844349721(float satisfaction, bool handoverByPlayer)
		{
			if (!base.IsServerInitialized)
			{
				NetworkManager networkManager = base.NetworkManager;
		...
		private void RpcLogic___ProcessHandoverClient_844349721(float satisfaction, bool handoverByPlayer)
		{
			TimeSinceLastDealCompleted = 0;
			if (satisfaction >= 0.5f)
			{
				ContractWellReceived(handoverByPlayer);
			}
		...
		private void RpcReader___Observers_ProcessHandoverClient_844349721(PooledReader PooledReader0, Channel channel)
		{
			float satisfaction = PooledReader0.ReadSingle();
			bool handoverByPlayer = PooledReader0.ReadBoolean();
			if (base.IsClientInitialized)
			{
				RpcLogic___ProcessHandoverClient_844349721(satisfaction, handoverByPlayer);
			}
		}
		...
		public virtual bool ReadSyncVar___ScheduleOne_002EEconomy_002ECustomer(PooledReader PooledReader0, byte UInt321, bool Boolean2)
		{
			if (UInt321 == 0)
			{
				if (PooledReader0 == null)
				{
					syncVar____003CCurrentAddiction_003Ek__BackingField.SetValue(syncVar____003CCurrentAddiction_003Ek__BackingField.GetValue(calledByUser: true), true);
					return true;
				}
				float value = PooledReader0.ReadSingle();
				syncVar____003CCurrentAddiction_003Ek__BackingField.SetValue(value, Boolean2);
				return true;
			}
			return false;
		}
