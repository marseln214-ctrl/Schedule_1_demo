...
namespace ScheduleOne.PlayerScripts
{
	public class ViewmodelSway : PlayerSingleton<ViewmodelSway>
	{
		...
		protected void UpdateJump()
		{
			...
			{
				timeSinceJumpStart = 0f;
				Vector3 b2 = new Vector3(0f, landCurve.Evaluate(Mathf.Clamp(timeSinceLanded / landJoltTime, 0f, 1f)) * landJoltMultiplier, 0f);
				if (landJoltMultiplier > 0f)
				{
					landPos = Vector3.Lerp(landPos, b2, Mathf.Abs(Time.deltaTime * landJoltSmooth / landJoltMultiplier));
				}
				else
				{
