...
namespace VLB
{
	...
	public class BeamGeometryHD : BeamGeometryAbstractBase
	{
		...
		private MaterialManager.StaticPropertiesHD ComputeMaterialStaticProperties()
		{
			...
			MaterialManager.StaticPropertiesHD result = default(MaterialManager.StaticPropertiesHD);
			result.blendingMode = (MaterialManager.BlendingMode)m_Master.blendingMode;
			result.attenuation = ((m_Master.attenuationEquation != 0) ? MaterialManager.HD.Attenuation.Quadratic : MaterialManager.HD.Attenuation.Linear);
			result.noise3D = (isNoiseEnabled ? MaterialManager.Noise3D.On : MaterialManager.Noise3D.Off);
			result.colorGradient = colorGradient;
			result.shadow = ((m_Shadow != null) ? MaterialManager.HD.Shadow.On : MaterialManager.HD.Shadow.Off);
			result.cookie = ((m_Cookie != null) ? ((m_Cookie.channel != CookieChannel.RGBA) ? MaterialManager.HD.Cookie.SingleChannel : MaterialManager.HD.Cookie.RGBA) : MaterialManager.HD.Cookie.Off);
			result.raymarchingQualityIndex = m_Master.raymarchingQualityIndex;
			return result;
		}
