...
namespace VLB
{
	...
	public class BeamGeometrySD : BeamGeometryAbstractBase, MaterialModifier.Interface
	{
		...
		private MaterialManager.StaticPropertiesSD ComputeMaterialStaticProperties()
		{
			...
			MaterialManager.StaticPropertiesSD result = default(MaterialManager.StaticPropertiesSD);
			...
			result.depthBlend = (isDepthBlendEnabled ? MaterialManager.SD.DepthBlend.On : MaterialManager.SD.DepthBlend.Off);
			result.colorGradient = colorGradient;
			result.dynamicOcclusion = m_Master._INTERNAL_DynamicOcclusionMode_Runtime;
			result.meshSkewing = (m_Master.hasMeshSkewing ? MaterialManager.SD.MeshSkewing.On : MaterialManager.SD.MeshSkewing.Off);
			result.shaderAccuracy = ((m_Master.shaderAccuracy != 0) ? MaterialManager.SD.ShaderAccuracy.High : MaterialManager.SD.ShaderAccuracy.Fast);
			return result;
		}
