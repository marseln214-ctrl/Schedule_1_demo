		...
		public static Mesh GenerateConeZ_Radii_DoubleCaps(float lengthZ, float radiusStart, float radiusEnd, int numSides, bool inverted)
		{
			...
			for (int i = 0; i < numSides; i++)
			{
				...
				for (int j = 0; j < 2; j++)
				{
					float num5 = j;
					float num6 = Mathf.Lerp(radiusStart, radiusEnd, num5);
					array[i + vertSidesStartFromSlide(j)] = new Vector3(num6 * num3, num6 * num4, num5 * lengthZ);
				}
			}
			array[vertCenterFromSlide(0)] = Vector3.zero;
			array[vertCenterFromSlide(1)] = new Vector3(0f, 0f, lengthZ);
			mesh.vertices = array;
			int num7 = numSides * 2 * 3;
			num7 += numSides * 3;
			num7 += numSides * 3;
			int[] indices = new int[num7];
			int ind = 0;
			for (int k = 0; k < numSides; k++)
			{
				int num8 = k + 1;
				...
				for (int l = 0; l < 1; l++)
				{
					int num9 = l * numSides;
					indices[ind] = num9 + k;
					indices[ind + (inverted ? 1 : 2)] = num9 + num8;
					indices[ind + ((!inverted) ? 1 : 2)] = num9 + k + numSides;
					indices[ind + 3] = num9 + num8 + numSides;
					indices[ind + (inverted ? 4 : 5)] = num9 + k + numSides;
					indices[ind + (inverted ? 5 : 4)] = num9 + num8;
					ind += 6;
				}
			}
			Action<int, bool> obj = delegate(int slideID, bool invert)
			{
				int num10 = vertSidesStartFromSlide(slideID);
				for (int m = 0; m < numSides - 1; m++)
				{
					indices[ind] = vertCenterFromSlide(slideID);
					indices[ind + (invert ? 1 : 2)] = num10 + m + 1;
					indices[ind + ((!invert) ? 1 : 2)] = num10 + m;
					ind += 3;
				}
				indices[ind] = vertCenterFromSlide(slideID);
				indices[ind + (invert ? 1 : 2)] = num10;
				indices[ind + ((!invert) ? 1 : 2)] = num10 + numSides - 1;
				ind += 3;
			};
			obj(0, inverted);
			obj(1, !inverted);
